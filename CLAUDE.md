...
4. `App` tracks a `collapsed` boolean and an `expandedSize` ref (persisted to `localStorage` under `gitbar.expandedSize` so a manual resize survives collapse cycles and app restarts). `toggleCollapsed` measures the header's live height via `getBoundingClientRect()` at click time, then issues a single `setSize(new LogicalSize(...))`; there's no `requestAnimationFrame` in this path. `onResized` is the sole writer of `collapsed`: it converts the resize payload to logical pixels via the current scale factor and compares against the live header height, so the chevron always reflects the actual OS window height regardless of what triggered the resize.
...
- **Dynamic colors from outside our palette have two paths.** **GitHub issue/PR label colors** go through the `.label-pill` CSS pattern in `src/styles/index.css`: set `--label-hue` to the raw `#hex` in the element's inline style (see `IssueCard.tsx`), and the stylesheet derives a tinted background, border, and foreground via `color-mix(in srgb, var(--label-hue) X%, …)`, with a `prefers-color-scheme: light` variant. No per-render JS color math, consistent palette across labels. For any **other** dynamic color we don't control (a hypothetical user-chosen theme tint, etc.), use `pickReadableTextColor()` in `src/lib/contrast.ts` as the fallback — YIQ luminance picks black or white. Static palette colors defined in CSS vars are already paired with readable text and don't need either.
- **Delivery protocol** (see `AGENTS.md` for details): non-trivial maintainer tasks write a `brief.md` and `delivery.md` to the **private companion repo** `pravton/gitbar-internal` (typically cloned to `~/Projects/gitbar-internal/`), not to this repo. `artifacts/` and `IN_PROGRESS.md` are gitignored here so they can't be re-introduced by accident. Skip the protocol entirely for tiny changes and for public contributor PRs.
